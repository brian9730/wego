...
function EditPost() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/posts/${id}`)
      .then(res => {
        const { name, category, region_type, region, content, image, author } = res.data;
        setForm({
          name,
  ...
  const handleSubmit = async e => {
    ...
    const confirmEdit = window.confirm('정말 게시글을 수정하시겠습니까?');
  ...
  const handleCancel = () => {
    if (window.confirm('수정을 취소하면 작성 중인 내용이 사라집니다. 계속하시겠습니까?')) {
      navigate(-1);
    }
  };
  ...
  return (
    <form className="post-form" onSubmit={handleSubmit}>
      ...
      {/* 기존 이미지 미리보기 */}
      {originalImage && !form.thumbnailFile && (
      <img
      ...
      <div className="post-form-buttons">
        <button type="submit" className="form-button">수정</button>
        <button type="button" onClick={handleCancel} className="form-button cancel-button">취소</button>
      </div>
    </form>
  );
}
